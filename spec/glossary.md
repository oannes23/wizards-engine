...
### User (Player Account)
A person who interacts with the system. Has an ID (ULID), display name (1–50 chars, trimmed, non-empty, editable via `PATCH /api/v1/me`), role (`gm`, `player`, or `viewer`), login code (plaintext, indexed), optional linked character ID, and active status. Starring stored in `starred_objects` table (not on User). The GM is a User with `role = gm` — not a separate entity. GM display name is shown as "GM [name]". GM can optionally own a character via `POST /api/v1/me/character`. Players are strictly 1:1 with characters. Viewers have no linked character. Account lifecycle tied to invite: new character requires new invite and new account (old deactivated). When deactivated, pending proposals are orphaned in place (GM decides their fate). GM can regenerate any user's login code (including their own); new magic link URL returned in response. Any user can refresh their own magic link via `POST /api/v1/me/refresh-link`. Auth: cookie-only (httpOnly, set by visiting magic link `/login/<code>` via `POST /api/v1/auth/login`). Auth errors: 401 (missing/invalid cookie) + 403 (insufficient role), both with JSON error body containing machine-readable `code` field. `GET /me` response includes `can_view_gm_content` and `can_take_gm_actions` capability flags. All users can see the player roster; GM callers see all users with `login_url`; viewer callers see all users without `login_url`; player callers see only non-viewer users without `login_url`.
...
### Invite Code
A single-use ULID generated by the GM via `POST /api/v1/game/invites`. The invite's `id` IS the shareable code — no separate `code` column. The invite carries a `role` field (`"player"` or `"viewer"`, default `"player"`). The invite is **bare** — not pre-linked to a character. GM shares `/login/<id>` with the recipient. On first visit, `POST /api/v1/auth/login` detects the unconsumed invite and returns `{type: "invite"}`. The recipient then joins via `POST /api/v1/game/join` with `{code, display_name, character_name?}`. For player invites: `character_name` is required — the system atomically creates a full Character (all mechanical fields defaulting to 0), a User account (login code = the invite id), and links them. For viewer invites: `character_name` is omitted — the system creates only a User account with no linked character. Sets the auth cookie. The same magic link now works as a permanent login. Consumed on redemption — cannot be reused. No expiry. GM can delete unconsumed invites.

### Viewer
A read-only spectator role with GM-level visibility. Viewers see all events (except `silent` system events), all proposals, the GM dashboard, and the full player roster. They cannot take any mutating actions (approve, reject, create, edit, delete). Viewer accounts have no linked character. Created via invites with `role: "viewer"`.
