...
**Participant records**: `{session_id, character_id, additional_contribution: bool}`. No `distributed` flag — see [downtime.md](downtime.md) for rationale. The owning player is derived from `character.user` — there is no stored `player_id` column.
...
- `visibility_level`: optional stored override of the default visibility (stored column; `null` means use the default `familiar`).
...
- `POST /api/v1/characters` — create (GM). Always creates a `simplified` (NPC) character — accepts `name`, `description`, `notes`, `attributes`. Full (PC) characters are created via the invite/join flow, not this endpoint.
...
_Last updated: 2026-03-17 (verified against Phase 2–4 implementation; corrected Story field list, participant record shape, and Character POST semantics)_
