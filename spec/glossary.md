...
An action that bypasses the proposal workflow — the player triggers it directly and an Event is created immediately. Three types: `find_time` (3 Plot → 1 FT), `use_effect` (decrement charge on a Magic Effect), `retire_effect` (retire a Magic Effect). See [actions.md](domains/actions.md).
